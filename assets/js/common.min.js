function fullPage() {
    $("#js-fullpage").fullpage({
        ...
        scrollingSpeed: 1e3,
        anchors: ["top", "resume", "about", "contact"],
        navigation: !0,
...
$(window).bind('mousewheel keyup keydown', async  function(e){
    await  sleep(1000);
    var hash = $(location).attr('hash')
    if(hash){
        if(hash =='#resume'){
            $('.video-over').get(0).play();
            $('.video-over').get(1).pause();
            $('.video-over').get(2).pause();
        }else if(hash =='#about'){

            $('.video-over').get(1).play();
            $('.video-over').get(0).pause();
            $('.video-over').get(2).pause();
        }else if(hash =='#contact'){

            $('.video-over').get(2).play();
            $('.video-over').get(1).pause();
            $('.video-over').get(0).pause();
        }
    }
...
$('.video-over'), $(window).on("load", function () {
    anime.timeline().add({
        targets: ".loader",
        translateY: [0, "-100%"],
        translateZ: 0,
...
$(function () {
    $win.on("load resize", function () {
        isSp = !0, fpnav = {y: "100%", x: 0}, image = {
            ...
            marginRight: 40,
            marginLeft: "auto"
        }, imageBig = {width: "60.677%", height: "100vh", top: "auto", marginLeft: "auto"}, console.log("tb" + isSp))
        var vids = $("video");
        $.each(vids, function(){
            this.controls = false;
        });
...
var underLayer = Barba.BaseView.extend({
    namespace: "underlayer", onEnter: function () {
        ...
        headerColor();
        anime.timeline({duration: 500, easing: "easeInOutCubic"}).add({
            targets: ".btn-wrap",
            translateY: ["110%", 0]
        }).add({targets: ".back-arrow svg", translateX: ["100%", 0], offset: "-=500"}).add({
            targets: ".scrollDown",
            translateY: ["180%", 0],
            offset: "-=500"
        })
    ...
    start: function () {
        ...
        return new Promise(function (resolve) {
            anime.timeline({
                ...
                translateZ: 0,
                offset: "-=500"
            }).add({targets: ".active .btn-wrap", translateY: [0, "110%"], translateZ: 0, offset: "-=500"}).play()
        })
    }, finish: function () {
        this.done()
    }
